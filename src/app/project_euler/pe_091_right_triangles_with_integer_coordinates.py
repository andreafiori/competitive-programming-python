...
class RightTriangleWithIntegerCoordinates:

    def is_right(self, x1: int, y1: int, x2: int, y2: int) -> bool:
        """
    ...
    def solution(self, limit: int = 50) -> int:
        """
        ...
        >>> RightTriangleWithIntegerCoordinates().solution(2)
        14
        >>> RightTriangleWithIntegerCoordinates().solution(10)
        448
        """
        return sum(
            1
            for pt1, pt2 in combinations(product(range(limit + 1), repeat=2), 2)
            if self.is_right(*pt1, *pt2)
        )
