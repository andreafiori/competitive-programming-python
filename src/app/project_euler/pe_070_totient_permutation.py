...
class TotientPermutation:
    ...
    def get_totients(self, max_one: int) -> list[int]:
        """
        Calculates a list of totients from 0 to max_one exclusive, using the
        definition of Euler's product formula.

        >>> TotientPermutation().get_totients(5)
        [0, 1, 1, 2, 2]

        >>> TotientPermutation().get_totients(10)
        [0, 1, 1, 2, 2, 4, 2, 6, 4, 6]
        """
    ...
    def has_same_digits(self, num1: int, num2: int) -> bool:
        """
        Return True if num1 and num2 have the same frequency of every digit, False
        otherwise.

        >>> TotientPermutation().has_same_digits(123456789, 987654321)
        True

        >>> TotientPermutation().has_same_digits(123, 23)
        False

        >>> TotientPermutation().has_same_digits(1234566, 123456)
        False
        """
        return sorted(str(num1)) == sorted(str(num2))


    def solution(self, max_n: int = 10000000) -> int:
        """
        Finds the value of n from 1 to max such that n/φ(n) produces a minimum.

        >>> TotientPermutation().solution(100)
        21

        >>> TotientPermutation().solution(10000)
        4435
        """

        min_numerator = 1  # i
        min_denominator = 0  # φ(i)
        totients = self.get_totients(max_n + 1)

        for i in range(2, max_n + 1):
            t = totients[i]

            if i * min_denominator < min_numerator * t and self.has_same_digits(i, t):
                min_numerator = i
                min_denominator = t
