...
class Magic5GonRing:
    ...
    def solution(self, gon_side: int = 5) -> int:
        """
        ...
        for perm in permutations(small_numbers + big_numbers):
            numbers = self.generate_gon_ring(gon_side, list(perm))
            if self.is_magic_gon(numbers):
    ...
    def generate_gon_ring(self, gon_side: int, perm: list[int]) -> list[int]:
        """
        Generate a gon_side-gon ring from a permutation state
        The permutation state is the ring, but every duplicate is removed

        >>> Magic5GonRing().generate_gon_ring(3, [4, 2, 3, 5, 1, 6])
        [4, 2, 3, 5, 3, 1, 6, 1, 2]
        >>> Magic5GonRing().generate_gon_ring(5, [6, 5, 4, 3, 2, 1, 7, 8, 9, 10])
    ...
    def is_magic_gon(self, numbers: list[int]) -> bool:
        """
        ...
        >>> Magic5GonRing().is_magic_gon([4, 2, 3, 5, 3, 1, 6, 1, 2])
        True
        >>> Magic5GonRing().is_magic_gon([4, 3, 2, 6, 2, 1, 5, 1, 3])
        True
        >>> Magic5GonRing().is_magic_gon([2, 3, 5, 4, 5, 1, 6, 1, 3])
        True
        >>> Magic5GonRing().is_magic_gon([1, 2, 3, 4, 5, 6, 7, 8, 9])
        False
        >>> Magic5GonRing().is_magic_gon([1])
        Traceback (most recent call last):
