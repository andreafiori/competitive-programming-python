...
class PowerfulDigitCounts:
    ...
    def solution(self, max_base: int = 10, max_power: int = 22) -> int:
        """
        Returns the count of all n-digit numbers which are nth power
        >>> PowerfulDigitCounts().solution(10, 22)
        49
        >>> PowerfulDigitCounts().solution(0, 0)
        0
        >>> PowerfulDigitCounts().solution(1, 1)
        0
        >>> PowerfulDigitCounts().solution(-1, -1)
        0
        """
