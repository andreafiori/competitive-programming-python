"""
Sum of digits sequence | 551 https://projecteuler.net/problem=551
...
class SumOfDigitsSequence:
    def __init__(self):
        self.ks = range(2, 20 + 1)
        self.base = [10**k for k in range(self.ks[-1] + 1)]
        self.memo: dict[int, dict[int, list[list[int]]]] = {}

    def next_term(self, a_i: list[int], k: int, i: int, n: int) -> tuple[int, int]:
        """
        ...
        c = sum(a_i[j] * self.base[j] for j in range(min(len(a_i), k)))

        diff, dn = 0, 0
        max_dn = n - i

        sub_memo = self.memo.get(ds_b)
        ...
        if sub_memo is not None:
            ...
            if jumps is not None and len(jumps) > 0:
                ...
                if max_jump >= 0:
                    ...
                    if new_c > 0:
                        self.add(a_i, k, new_c)

            else:
                sub_memo[c] = []
        else:
            sub_memo = {c: []}
            self.memo[ds_b] = sub_memo

        if dn >= max_dn or c + diff >= self.base[k]:
            return diff, dn

        if k > self.ks[0]:
            while True:
                # keep doing smaller jumps
                _diff, terms_jumped = self.next_term(a_i, k - 1, i + dn, n)
                diff += _diff
                dn += terms_jumped
            ...
            _diff, terms_jumped = self.compute(a_i, k, i + dn, n)
            diff += _diff
            dn += terms_jumped
    ...
    def compute(self, a_i: list[int], k: int, i: int, n: int) -> tuple[int, int]:
        """
        ...
        if addend > 0:
            self.add(a_i, k, addend)
        return diff, i - start_i


    def add(self, digits: list[int], k: int, addend: int) -> None:
        """
    ...
    def solution(self, n: int = 10**15) -> int:
        """
        ...
        dn = 0
        while True:
            _diff, terms_jumped = self.next_term(digits, 20, i + dn, n)
            dn += terms_jumped
            if dn == n - i:
                break
