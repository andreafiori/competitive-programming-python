"""
Project Euler 587 | https://projecteuler.net/problem=587
...
class ConcaveTriangle:
    ...
    def solution(self, fraction: float = 1 / 1000) -> int:
        ...
        for n in count(1):
            if self.concave_triangle_area(n) / l_section_area < fraction:
                return n

        return -1

    def circle_bottom_arc_integral(self, point: float) -> float:
        """
        Returns integral of circle bottom arc y = 1 / 2 - sqrt(1 / 4 - (x - 1 / 2) ^ 2)

        >>> ConcaveTriangle().circle_bottom_arc_integral(0)
        0.39269908169872414

        >>> ConcaveTriangle().circle_bottom_arc_integral(1 / 2)
        0.44634954084936207

        >>> ConcaveTriangle().circle_bottom_arc_integral(1)
        0.5
        """
    ...
    def concave_triangle_area(self, circles_number: int) -> float:
        """
        Returns area of concave triangle

        >>> ConcaveTriangle().concave_triangle_area(1)
        0.026825229575318944

        >>> ConcaveTriangle().concave_triangle_area(2)
        0.01956236140083944
        """
        ...
        concave_region_area = self.circle_bottom_arc_integral(
            1 / 2
        ) - self.circle_bottom_arc_integral(intersection_x)
