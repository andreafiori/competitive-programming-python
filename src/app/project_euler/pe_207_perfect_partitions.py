...
Problem 207: Perfect partitions | https://projecteuler.net/problem=207
...
class PerfectPartitions:

    def check_partition_perfect(self, positive_integer: int) -> bool:
        """
        Check if t = f(positive_integer) = log2(sqrt(4*positive_integer+1)/2 + 1/2) is a
        real number.
        >>> PerfectPartitions().check_partition_perfect(2)
        True
        >>> PerfectPartitions().check_partition_perfect(6)
        False
        """
    ...
    def solution(self, max_proportion: float = 1 / 12345) -> int:
        """
        Find m for which the proportion of perfect partitions to total partitions is lower
        than max_proportion
        >>> PerfectPartitions().solution(1) > 5
        True
        >>> PerfectPartitions().solution(1/2) > 10
        True
        >>> PerfectPartitions().solution(3 / 13) > 185
        True
        """
        ...
        while True:
            ...
            if partition_candidate == int(partition_candidate):
                ...
                total_partitions += 1
                if self.check_partition_perfect(partition_candidate):
