...
class RomanNumerals:
    ...
    def parse_roman_numerals(self, numerals: str) -> int:
        ...
        while index < len(numerals) - 1:
            current_value = self.SYMBOLS[numerals[index]]
            next_value = self.SYMBOLS[numerals[index + 1]]
            if current_value < next_value:
                total_value -= current_value
            else:
                total_value += current_value
        total_value += self.SYMBOLS[numerals[index]]
        total_value += self.SYMBOLS[numerals[index]]

        return total_value


    def generate_roman_numerals(self, num: int) -> str:
        """
    ...
    def solution(self, roman_numerals_filename: str = "/p089_roman.txt") -> int:
        """
        ...
        for line in lines:
            original = line.strip()
            num = self.parse_roman_numerals(original)
            shortened = self.generate_roman_numerals(num)
