...
class GreatestProduct:
    ...
    def largest_product(self, grid: list[list[int]]) -> int:
    ...
    def solution_one(self) -> int:
        ...
        >>> GreatestProduct().solution_one()
        70600674
        """
        grid = []
        with open(os.path.dirname(__file__) + "/data/grid.txt") as file:
            for line in file:
                grid.append(line.strip("\n").split(" "))

        grid = [[int(i) for i in grid[j]] for j in range(len(grid))]

        return self.largest_product(grid)

    def solution_two(self) -> int:
        """Returns the greatest product of four adjacent numbers (horizontally,
        vertically, or diagonally).

        >>> GreatestProduct().solution_two()
        70600674
        """
        with open(os.path.dirname(__file__) + "/data/grid.txt") as f:
            grid = []
            for _ in range(20):
