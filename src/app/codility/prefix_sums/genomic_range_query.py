...
class GenomicRangeQuery:
    ...
    def slow_solution(self, S, P, Q):
        ...
        for p, q in zip(P, Q):
            ...
            result.append(self.M[slice[0]])
        return result

    def fast_solution(self, S, P, Q):
        """
        ...
        for p, q in zip(P, Q):
            if sumA[q+1] > sumA[p]:
                impact.append(self.M['A'])
            elif sumC[q+1] > sumC[p]:
                impact.append(self.M['C'])
            elif sumG[q + 1] > sumG[p]:
                impact.append(self.M['G'])
            else:
                impact.append(self.M['T'])
        return impact
