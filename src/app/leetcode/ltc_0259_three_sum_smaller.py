"""
Three Sum Smaller | leetcode 259 | https://leetcode.com/problems/3sum-smaller/
...
class ThreeSumSmaller:
    ...
    def solution_one(self, nums: list, target: int) -> int:
        """
        :type nums: List[int]
        :type target: int
        :rtype: int
        ...
        res = 0
        for i in range(ls - 1):
            res += self.solution_two(nums, i + 1, target - nums[i])
        return res

    def solution_two(self, nums: list, start: int, target: int) -> int:
